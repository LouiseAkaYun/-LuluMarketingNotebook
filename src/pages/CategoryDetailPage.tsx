...
import { Loader2, Tag, ArrowLeft, Package, FileText } from 'lucide-react';
...
const CategoryDetailPage = () => {
  ...
  const handleExportReport = async () => {
    ...
    try {
      ...
      const info = [
        { label: 'Project', value: category.projects?.name },
        { label: 'Description', value: category.description },
      ];

      exportReportToHtml(category.name, info, modulesWithRows.filter(m => !!m.definition));
    } catch (err) {
